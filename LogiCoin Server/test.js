...
MC4CAQAwBQYDK2VwBCIEIK0SwIWE42UB3iNI5zGJaYIX5q8SU7Hgl0wiPn5Nca9m
-----END PRIVATE KEY-----`;
  
  (async () => {
    const client = await createAuthenticatedClient({
      walletAddressUrl: "https://ilp.interledger-test.dev/mthoko", // Make sure the wallet address starts with https:// (not $), and has no trailing slashes
      privateKey: privateKey,
      keyId: "e165e80d-391c-42f0-91c4-61cfa1164257",
    });
  
    const sendingWalletAddress = await client.walletAddress.get({
      url: "https://ilp.interledger-test.dev/mthoko", // Make sure the wallet address starts with https:// (not $)
    });
    const receivingWalletAddress = await client.walletAddress.get({
      url: "https://ilp.interledger-test.dev/zulu", // Make sure the wallet address starts with https:// (not $)
    });
